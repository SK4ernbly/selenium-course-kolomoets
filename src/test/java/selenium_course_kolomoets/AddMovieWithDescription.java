...
public class AddMovieWithDescription extends TestBase {
  ...
  public void AddMovie() throws Exception {
    ...
    //проверка, что есть подсказка о том, что поле "name" обязательное:
    if (!isElementPresent(By.xpath(".//*[@id='updateform']/table/tbody/tr[2]/td[2]/label"))){
    	System.out.println("Element not found");
    	String e = null;
		throw new NoSuchElementException(e);   	
    }    
    //попытка отправить форму с незаполненными обяз. полями. Должны остаться на той же странице(т.е. остальные элементы должны находиться) 
    ...
    //проверка, что есть подсказка о том, что поле "year" обязательное:
    if (!isElementPresent(By.xpath(".//*[@id='updateform']/table/tbody/tr[4]/td[2]/label"))){
    	System.out.println("Element not found");
    	String e = null;
		throw new NoSuchElementException(e);   	
    }
    //попытка отправить форму с незаполненным обяз.полем "year". Должны остаться на той же странице(т.е.остальные элементы должны находиться)
    driver.findElement(By.id("submit")).click();
